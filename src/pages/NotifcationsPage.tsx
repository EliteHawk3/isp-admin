...
const NotificationsPage = () => {
  ...
  const validateAndSendNotification = (title: string, body: string, validStatuses: string[]) => {
    const invalidUsers = selectedUsers.filter(
      (id) => !validStatuses.includes(users.find((user) => user.id === id)?.paymentStatus || "")
    );
  ...
  const handleSendNotification = (title: string, body: string) => {
    ...
    const newNotification: Notification = {
      ...
      status: "Sent",
      users: selectedUsers.map((id) => users.find((user) => user.id === id)?.name || "Unknown"),
    };
  ...
  return (
    <div className="flex flex-col bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white p-6">
      {/* Header */}
      <h1 className="text-3xl font-extrabold tracking-wide mb-6">Notifications</h1>
  ...
  <div className="flex items-center p-3 rounded-full shadow-md hover:shadow-lg transition-shadow duration-300">
    ...
    />
          ...
          <ul className="space-y-4 text-sm">
            {[
              { label: "All", count: userCounts.All, icon: FaUsers, color: "text-blue-500" },
              { label: "Pending", count: userCounts.Pending, icon: FaExclamationCircle, color: "text-yellow-500" },
              { label: "Paid", count: userCounts.Paid, icon: FaCheckCircle, color: "text-green-500" },
              { label: "Overdue", count: userCounts.Overdue, icon: FaClock, color: "text-red-500" },
            ].map((item, index) => (
              <li
                key={index}
                ...
                className={` border-b border-gray-700 flex items-center gap-3 cursor-pointer hover:bg-gray-700 hover:bg-opacity-50 p-3 rounded-lg transition shadow-sm ${
                  selectedCategory === item.label ? "bg-gray-700 bg-opacity-50" : ""
                }`}
              >
                <item.icon className={`${item.color} text-lg`} />
                <span>{item.label}</span>
                <span className="ml-auto text-gray-400 text-sm">{item.count}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <tbody>
        {filteredUsers.map((user) => (
          ...
          >
            ...
            {selectedUsers.includes(user.id) && <FaCheckCircle className="text-green-500" />}
            {user.name}
            </td>
      ...
      >
        ...
        <div>
          <p className="text-sm text-gray-400">
            Sent to <span className="font-bold">{notification.users.length}</span> user(s):
          </p>
          ...
          <div className="bg-gray-900 p-8 rounded-xl shadow-lg text-white w-1/3">
            ...
            <div className="flex justify-end gap-4">
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                onClick={() => {
                  handleSendNotification(customNotification.title, customNotification.body);
